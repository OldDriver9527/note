...
public class LearnSpringIoc {
	 ...
	 * spring容器会扫描基础包及其子包，创建组件类对象，进行装配后，加入spring容器
	 * 相关注解
	 ...
	 * 
	 * 在servlet中使用@Autowired
	 * 默认servlet不在spring 容器管理范围，因此无法在servlet中使用自动装配
	 * 可以通过SpringBeanAutowiringSupport的processInjectionBasedOnCurrentContext，
	 * 为servlet添加自动装配功能
	 */ 
}
